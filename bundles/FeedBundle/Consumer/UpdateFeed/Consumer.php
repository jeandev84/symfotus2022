...
class Consumer implements ConsumerInterface
{
    ...
    public function execute(AMQPMessage $msg): int
    {
        ...
        try {
            ...
            // Стимулируем проблема, которая может произойти
            /*if ($message->getFollowerId() === 5) {
                sleep(2);
                throw new Exception();
            }*/

            // Транзакционная отправка сообщения
    ...
    /* public function executeUnTransactionalSendMessage(AMQPMessage $msg): int
    {
        try {
            $message = Message::createFromQueue($msg->getBody());
            $errors = $this->validator->validate($message);
        ...
        // Не-транзакционная отправка сообщения
